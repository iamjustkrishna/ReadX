...
class DynamicAiModelConfigRepository(context: Context) {
    ...
    fun sanitizeModel(provider: AiProvider, currentModel: String): String {
        if (DECOMMISSIONED_MODELS.contains(currentModel.trim())) {
            return getDefaultModelFor(provider)
        }
        val available = getModelsFor(provider).map { it.first }
        return if (available.contains(currentModel.trim())) {
            currentModel.trim()
        } else {
    ...
    private fun parseAndApply(jsonStr: String): Boolean {
        return runCatching {
            ...
            if (version < MIN_CONFIG_VERSION) {
                // Invalidate stale cache with older decommissioned models
            ...
            for (provider in AiProvider.values()) {
                ...
                if (modelsArr != null) {
                    for (i in 0 until modelsArr.length()) {
                        ...
                        if (!DECOMMISSIONED_MODELS.contains(id)) {
                ...
                if (modelList.isNotEmpty()) {
                    ...
                    memoryDefaultModels[provider] = if (DECOMMISSIONED_MODELS.contains(defModel)) {
                        getHardcodedDefaultModel(provider)
                    } else {
                        defModel
                    }
                }
    ...
    companion object {
        ...
        private const val MIN_CONFIG_VERSION = 3

        val DECOMMISSIONED_MODELS = setOf(
        ...
        fun getHardcodedModels(provider: AiProvider): List<Pair<String, String>> = when (provider) {
            AiProvider.GROQ -> listOf(
                "openai/gpt-oss-120b" to "GPT OSS 120B (High Intelligence)",
                "openai/gpt-oss-20b" to "GPT OSS 20B (Fast & Efficient)",
                "qwen/qwen3.6-27b" to "Qwen 3.6 27B (Reasoning)",
                "groq/compound" to "Compound (Advanced)",
                "groq/compound-mini" to "Compound Mini (Ultra Fast)"
            )
